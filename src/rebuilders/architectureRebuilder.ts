...
interface ArchService { id: string; in?: string; icon?: string; label: string; }
interface ArchGroup { id: string; in?: string; icon?: string; label: string; }
interface ArchEdge { lhs: string; lhsDir: string; rhsDir: string; rhs: string; label?: string; }
interface ArchModel { services: ArchService[]; groups: ArchGroup[]; edges: ArchEdge[]; }
...
function parseIconLabel(raw: string): { icon?: string; label: string } {
  // service id(icon)[label]  or  id[label]  or  id("icon")[label]
...
export function parseLooseArchitecture(code: string): ArchModel | null {
  ...
  const isArch = /^(architecture(-beta)?|Architecture|arch\b|archDiagram|systemArchitecture|infraDiagram|deploymentDiagram|cloudArchitecture|microserviceDiagram|awsDiagram|gcpDiagram|azureDiagram)/i.test(lines[0]);
  ...
  for (let i = 1; i < lines.length; i++) {
    const line = lines[i];
    if (!line || line.startsWith('%%') || line === '}') { if (line === '}') currentGroup = undefined; continue; }
    ...
    if (serviceMatch) {
      ...
      model.services.push({ id, icon, label, in: serviceMatch[2] ?? currentGroup });
      continue;
    }

    // edge: A{L} --> B{R}  OR  A:L --> B:R  OR  A -- B (loose)
    // Mermaid arch edge format: lhsId{dir} -- [label] --> rhsId{dir}
    const edgeFull = line.match(/^(\w+)\{([LRTB])\}\s*(?:--\s*"?([^"]*)"?\s*)?-->\s*(\w+)\{([LRTB])\}/);
    if (edgeFull) {
      model.edges.push({ lhs: edgeFull[1], lhsDir: edgeFull[2], rhsDir: edgeFull[5], rhs: edgeFull[4], label: edgeFull[3]?.trim() });
      continue;
    }

    // loose edge: A --> B  /  A -- B  /  A -> B
    const looseEdge = line.match(/^(\w+)\s*(?:-->|->|--)\s*(\w+)(?:\s*:\s*(.+))?$/);
    if (looseEdge) {
      model.edges.push({ lhs: looseEdge[1], lhsDir: 'R', rhsDir: 'L', rhs: looseEdge[2], label: looseEdge[3]?.trim() });
      continue;
    }

    // bare label → treat as unnamed service
    const bareId = line.match(/^(\w+)\["([^"]+)"\]$/);
    if (bareId) {
      model.services.push({ id: bareId[1], label: bareId[2], in: currentGroup });
    }
  }
...
export function buildArchitecture(model: ArchModel): string {
  ...
  for (const e of model.edges) {
    ...
    lines.push(`  ${e.lhs}{${e.lhsDir}} --${labelStr}--> ${e.rhs}{${e.rhsDir}}`);
  }
...
export const architectureRebuilderPass = {
  ...
  repair(ctx: RepairContext): RepairResult {
    ...
    return { passName: this.name, changed, code: rebuilt, repairs: changed ? [`Rebuilt architecture-beta (${model.services.length} services, ${model.edges.length} connections)`] : [] };
  },
};
