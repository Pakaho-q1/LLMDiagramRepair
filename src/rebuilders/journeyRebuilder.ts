// rebuilders/journeyRebuilder.ts
import type { RepairContext, RepairResult, DiagramKind } from '../types/index.js';

interface JourneyTask { name: string; score: number; actors: string[]; }
interface JourneySection { name: string; tasks: JourneyTask[]; }
interface JourneyModel { title: string; sections: JourneySection[]; }
...
export function parseLooseJourney(code: string): JourneyModel | null {
  ...
  const isJourney = /^(journey|userJourney|user[-_]journey|User Journey|customerJourney|customer[-_]journey)/i.test(lines[0]);
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // OR:   Name : score  (ไม่มี actor → default Me)
    // OR:   Name (score) Actor  (loose)
    const taskFull = line.match(/^([^:]+?)\s*:\s*(\d+)\s*:\s*(.+)$/);
    if (taskFull) {
      const actors = taskFull[3].split(',').map(a => stripQuotes(a.trim())).filter(Boolean);
      currentSection.tasks.push({ name: stripQuotes(taskFull[1]), score: parseInt(taskFull[2]), actors });
      continue;
    }

    const taskNoActor = line.match(/^([^:]+?)\s*:\s*(\d+)\s*$/);
    if (taskNoActor) {
      currentSection.tasks.push({ name: stripQuotes(taskNoActor[1]), score: parseInt(taskNoActor[2]), actors: ['Me'] });
      continue;
    }

    // loose: Name (score)  Actor
    const looseTask = line.match(/^(.+?)\s*\((\d+)\)\s*(.*)$/);
    if (looseTask) {
      const actors = looseTask[3] ? looseTask[3].split(/[,\s]+/).filter(Boolean) : ['Me'];
      currentSection.tasks.push({ name: stripQuotes(looseTask[1].trim()), score: parseInt(looseTask[2]), actors });
      continue;
    }
  }
...
export function buildJourney(model: JourneyModel): string {
  ...
  for (const section of model.sections) {
    if (model.sections.length > 1 || section.name !== 'Journey') lines.push(`  section ${section.name}`);
...
export const journeyRebuilderPass = {
  ...
  repair(ctx: RepairContext): RepairResult {
    ...
    return { passName: this.name, changed, code: rebuilt, repairs: changed ? [`Rebuilt journey (${tasks} tasks, ${model.sections.length} sections)`] : [] };
  },
};
