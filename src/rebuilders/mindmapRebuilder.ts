// rebuilders/mindmapRebuilder.ts
import type { RepairContext, RepairResult, DiagramKind } from '../types/index.js';

interface MindNode { text: string; shape?: string; icon?: string; class?: string; children: MindNode[]; depth: number; }

function stripQuotes(s: string) { return s.replace(/^["'`]|["'`]$/g, '').trim(); }

function detectShape(raw: string): { text: string; shape?: string } {
  if (raw.startsWith('((') && raw.endsWith('))')) return { text: raw.slice(2, -2), shape: '((' };
  if (raw.startsWith('(') && raw.endsWith(')')) return { text: raw.slice(1, -1), shape: '(' };
  if (raw.startsWith('[') && raw.endsWith(']')) return { text: raw.slice(1, -1), shape: '[' };
  if (raw.startsWith('{') && raw.endsWith('}')) return { text: raw.slice(1, -1), shape: '{' };
  if (raw.startsWith('{{') && raw.endsWith('}}')) return { text: raw.slice(2, -2), shape: '{{' };
  if (raw.startsWith('>') && raw.endsWith(']')) return { text: raw.slice(1, -1), shape: '>' };
...
export function parseLooseMindmap(code: string): MindNode | null {
  ...
  const isMindmap = /^(mindmap|mindMap|mind[-_]map|MindMap|Mind Map|mm\b)/i.test(lines[0].trim());
  if (!isMindmap) return null;

  // หา root node (บรรทัดแรกหลัง header ที่ไม่ใช่ directive)
  let rootLine = -1;
  for (let i = 1; i < lines.length; i++) {
    const t = lines[i].trim();
    if (t && !t.startsWith('%%') && !t.startsWith('::icon') && !t.startsWith(':::')) {
      rootLine = i;
      break;
    }
  }
  ...
  for (let i = rootLine; i < lines.length; i++) {
    ...
    if (!root) { root = node; stack.push(node); continue; }

    // pop stack จนเจอ parent ที่ depth น้อยกว่า
...
export const mindmapRebuilderPass = {
  ...
  repair(ctx: RepairContext): RepairResult {
    ...
    if (!root) return { passName: this.name, changed: false, code: ctx.code, repairs: [] };
    const rebuilt = buildMindmap(root);
    const changed = rebuilt !== ctx.code;
    function countNodes(n: MindNode): number { return 1 + n.children.reduce((s, c) => s + countNodes(c), 0); }
    return { passName: this.name, changed, code: rebuilt, repairs: changed ? [`Rebuilt mindmap (${countNodes(root)} nodes)`] : [] };
  },
};
