// rebuilders/kanbanRebuilder.ts
import type { RepairContext, RepairResult, DiagramKind } from '../types/index.js';

interface KanbanCard { id?: string; label: string; assigned?: string; }
interface KanbanColumn { label: string; cards: KanbanCard[]; }
interface KanbanModel { columns: KanbanColumn[]; }
...
export function parseLooseKanban(code: string): KanbanModel | null {
  ...
  const isKanban = /^(kanban|Kanban|kanban[-_]board|KanbanBoard)/i.test(lines[0]);
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // column header: @{ column: label }  OR  section Label  OR  ## Label  OR bare Label (all-caps / title-like)
    ...
    // mermaid kanban column: colId["label"]  or  colId
    ...
    // card: @{ id: "id", label: "label" }  OR  - label  OR  * label  OR  item label
    const itemMatch = line.match(/^[-*•]\s+(.+)$/);
    if (itemMatch) {
      const text = stripQuotes(itemMatch[1]);
      if (!currentColumn) { currentColumn = { label: 'Backlog', cards: [] }; model.columns.push(currentColumn); }
      currentColumn.cards.push({ label: text });
      continue;
    }

    // id["label"]  →  card inside current column (mermaid syntax)
    ...
    // @{ metadata } lines — skip for now
    if (line.startsWith('@{') || line.startsWith('}')) continue;

    // bare label line → treat as column if no current, else card
    if (!currentColumn) {
...
export function buildKanban(model: KanbanModel): string {
  ...
  for (const col of model.columns) {
    // slugify column label to id
    const colId = col.label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '') || 'col';
    lines.push(`  ${colId}["${col.label}"]`);
    for (const card of col.cards) {
      const cardId = card.id ?? (col.label.slice(0, 3) + Math.random().toString(36).slice(2, 6));
...
export const kanbanRebuilderPass = {
  ...
  repair(ctx: RepairContext): RepairResult {
    ...
    return { passName: this.name, changed, code: rebuilt, repairs: changed ? [`Rebuilt kanban (${model.columns.length} columns, ${cards} cards)`] : [] };
  },
};
