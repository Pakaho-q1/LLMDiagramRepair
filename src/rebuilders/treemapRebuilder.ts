// rebuilders/treemapRebuilder.ts
import type { RepairContext, RepairResult, DiagramKind } from '../types/index.js';

interface TreemapNode { label: string; value?: number; children: TreemapNode[]; }
...
export function parseLooseTreemap(code: string): TreemapNode | null {
  ...
  const isTreemap = /^(treemap(-beta)?|Treemap|tree[-_]map|TreeMap)/i.test(lines[0].trim());
  if (!isTreemap) return null;

  // find root
  let rootLine = -1;
  for (let i = 1; i < lines.length; i++) {
    if (lines[i].trim() && !lines[i].trim().startsWith('%%')) { rootLine = i; break; }
  }
  ...
  for (let i = rootLine; i < lines.length; i++) {
    ...
    // parse: "Label : value"  or  "Label[value]"  or  bare "Label"
    let label = trimmed;
    ...
    if (colonMatch) { label = stripQuotes(colonMatch[1]); value = parseFloat(colonMatch[2]); }
    else if (bracketMatch) { label = stripQuotes(bracketMatch[1]); value = parseFloat(bracketMatch[2]); }
    else { label = stripQuotes(trimmed); }

    const node: TreemapNode = { label, value, children: [] };

    if (!root) { root = node; stack.push({ node, depth }); continue; }
...
export const treemapRebuilderPass = {
  ...
  repair(ctx: RepairContext): RepairResult {
    ...
    function count(n: TreemapNode): number { return 1 + n.children.reduce((s, c) => s + count(c), 0); }
    return { passName: this.name, changed, code: rebuilt, repairs: changed ? [`Rebuilt treemap-beta (${count(root)} nodes)`] : [] };
  },
};
